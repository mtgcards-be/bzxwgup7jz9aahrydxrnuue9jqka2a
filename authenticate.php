...
if ($login_attempts && $login_attempts['attempts_left'] <= 0) {
	exit('Maximum amount of attempts reached. You cannot login right now, please try again later!');
}
...
if (!isset($_POST['username'], $_POST['password'])) {
	...
	exit('Please fill both email and password field!');
}
...
if ($account) {
	...
	if (password_verify($_POST['password'], $account['password'])) {
		...
		echo 'Please verify your password! You have ' . $login_attempts['attempts_left'] . ' attempts remaining!';
	}
} else {
	echo 'Unknown email!';
	$login_attempts = loginAttempts($pdo, TRUE);
	echo 'Incorrect email, you have ' . $login_attempts['attempts_left'] . ' attempts remaining!';
}
?>
